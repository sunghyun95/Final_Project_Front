...
function CurriculumUpdate({ match }) {
  const classes = useStyles();

  ////
  const [processData, setProcessData] = React.useState([]);
  const [processFiles, setProcessFiles] = React.useState([]);
  const [processBooks, setProcessBooks] = React.useState([]);
  useEffect(() => {
    console.log(match.params.process_num);
    let formUrl =
      "http://localhost:8000/project/process/updateform?process_num=" +
      match.params.process_num;
    axios
      .get(formUrl)
      .then((res) => {
        console.log(res.data.processdto);
        setProcess_subject(res.data.processdto.process_subject);
        setProcess_type(res.data.processdto.process_type);
        setProcess_startdate(res.data.processdto.process_startdate);
        setProcess_enddate(res.data.processdto.process_enddate);
        setProcess_peoples(res.data.processdto.process_peoples);
        setProcess_teacher(res.data.processdto.process_teacher);
        setProcess_intr(res.data.processdto.process_intr);

        console.log(res.data.processfiles);
        setProcessFiles(res.data.processfiles);
        console.log(res.data.books);
        setProcessBooks(res.data.books);
      })
      .catch((err) => {
        console.log("수정 폼 불러오기 에러 :" + err);
      });
  }, []);

  const [process_subject, setProcess_subject] = React.useState(
    processData.process_subject
  );
  ...
  const teachers = teacherList.map((item, index) => {
    return (
      <MenuItem key={index} value={item.member_num}>
        {item.member_name}
      </MenuItem>
    );
  });
  ...
  const ProcessSubmit = (e) => {
    ...
    data.append("process_num", match.params.process_num);
    data.append("process_type", process_type);
    data.append("process_member_num", 12);
    ...
    data.append("process_writer", "나야나");
    ...
    axios({
      method: "post",
      url: "http://localhost:8000/project/process/update",
      data: data,
    ...
    console.log("clickbooks:" + deleteClickBooks);
    console.log("file:" + deleteClickImages);

    let delUrl =
      "http://localhost:8000/project/process/deletefiles?deleteBooks=" +
      deleteClickBooks +
      "&deleteImage=" +
      deleteClickImages;
    axios
    ...
    if (bookTag.length > 0) {
      bookTag.map((item, index) => {
        ...
        bookdata.append("books_process_num", match.params.process_num);
  ...
  const [deleteClickBooks, setDeleteClickBooks] = React.useState([]);

  const clickBooks = []; //삭제할 제공 교제 책들
  const deleteProcessBooks = (num, id) => {
    console.log(num);
    document.getElementById(id).style.display = "none";
    clickBooks.push(num);
    setDeleteClickBooks(deleteClickBooks.concat(clickBooks));
    console.log("deleteClickBooks:" + deleteClickBooks);
  };

  const [deleteClickImages, setDeleteClickImages] = React.useState([]);
  const file = []; // 삭제할 커리큘럼이미지파일
  const deleteImages = (num, id) => {
    console.log(num);
    document.getElementById(id).style.display = "none";
    file.push(num);
    setDeleteClickImages(deleteClickImages.concat(file));
    console.log(deleteClickImages);
  };
  const tableStyle = {
    textAlign: "center",
    fontSize: "16px",
        ...
        <table style={tableStyle} align="center">
          ...
          <tbody>
                    ...
                    <span key={index} style={{ marginTop: "10px" }}>
                      <img
                        ...
                        src={
                          ...
                          item.processfiles_process_filename
                        }
                        style={{ height: "100px", width: "100px" }}
                        id={"delimage" + item.processfiles_num}
                        onClick={deleteImages.bind(
                          this,
                          item.processfiles_num,
                          "delimage" + item.processfiles_num
                        )}
                      />
                    </span>
                  );
            ...
            <tr style={trStyle}>
              <td rowSpan="3">교재</td>
              <td align="left" style={{ textAlign: "left" }}>
                {processBooks.map((item, index) => (
                  <span
                    key={index}
                    id={"delbooks" + item.books_num}
                    style={{ cursor: "pointer" }}
                    onClick={deleteProcessBooks.bind(
                      this,
                      item.books_num,
                      "delbooks" + item.books_num
                    )}
                  >
                    {item.books_name}{" "}
                  </span>
